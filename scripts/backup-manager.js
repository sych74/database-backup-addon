function BackupManager(config) {
    ...
    me.restore = function () {
		...
		'if [ "$COMPUTE_TYPE" == "postgres" ]; then rm -f /root/postgres-restore.sh; wget -O /root/postgres-restore.sh %(baseUrl)/scripts/postgres-restore.sh; chmod +x /root/postgres-restore.sh; bash /root/postgres-restore.sh 2> >(tee -a %(restoreLogFile) >&2); else true; fi',
