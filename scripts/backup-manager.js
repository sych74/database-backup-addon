function BackupManager(config) {
    ...
    me.restore = function () {
		...
		'if [ "$COMPUTE_TYPE" == "mysql" ] || [ "$COMPUTE_TYPE" == "percona" ]; then rm -f /root/mysql-restore.sh; wget -O /root/mysql-restore.sh %(baseUrl)/scripts/mysql-restore.sh; chmod +x /root/mysql-restore.sh; bash /root/mysql-restore.sh %(dbuser) %(dbpass) 2> >(tee -a %(restoreLogFile) >&2); else true; fi',
