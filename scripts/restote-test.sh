...
function get_dump_snapshot_id_by_name(){
    local backup_name="$1"
    local snapshot_id=$(RESTIC_PASSWORD=${ENV_NAME} restic -r /opt/backup/${ENV_NAME} snapshots --json | jq -r '.[] | select(.tags[0] | contains('$backup_name')) | select((.tags[1] | contains("BINLOGS") | not) // true) | .short_id')
...
function get_dump_snapshot_id_by_time(){
    local backup_time="$1"
...
function restore_snapshot_by_id(){
    local snapshot_id="$1"
    RESTIC_PASSWORD=${ENV_NAME} GOGC=20 restic -r /opt/backup/${ENV_NAME} restore ${snapshot_id} --target / || { echo $(date) ${ENV_NAME} "Error: Failed to restore snapshot ID $snapshot_id" | tee -a ${RESTORE_LOG_FILE}; exit 1; }
